...
class RedBean implements DataProvider
{
    ...
    /**
     * Convert an Array to storable data
     */
    protected function saveArray($DomainObject, $bean, $name, $value)
    {
        $arrTableName = $this->getArrayTableName($DomainObject, $name);
        $arrBean = R::xdispense($arrTableName);
        $arrLinkName = $this->getArrayRefName($DomainObject, $name);
        foreach($value as $_name => $_value) {
            $arrBean->$_name = json_encode($_value);
        }
        $bean->{$arrLinkName} = $arrBean;

        echo '<pre>' . print_r($arrBean, true) . '</pre>';
    }

    /**
     * Checks if an array has numeric Keys. Allows non-sequential arrays.
     * @param Array $array Mixed Indexed or Associative Array
     */
    ...
    /**
     * Array mapped to SQL table name
     */
    protected function getArrayTableName(DomainObject $DomainObject, $name)
    {
        return strtolower($this->tablePrefix . $DomainObject->getObjectName() . '_' . $name);
    }

    /**
     * Array reference name mapped to Redbean relation name
     */
    protected function getArrayRefName(DomainObject $RefObject, $name)
    {
        return $name = 'xown' . ucfirst($this->getArrayTableName($RefObject, $name)) . 'List';
    }
